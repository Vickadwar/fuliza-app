...
export default function QuickLoanPage() {
  ...
  const handleEntrySubmit = (e: React.FormEvent) => {
    ...
    const interval = setInterval(() => {
      ...
      if (current > 4) {
        ...
        setOtpSent(code);
        sendOTPSMS(formData.phone, code, 'LOAN');
        setStep('otp');
      }
    }, 1200);
  };
  ...
  const copyToClipboard = () => {
    if (receipt?.trackId) {
      navigator.clipboard.writeText(receipt.trackId);
      setCopied(true);
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20 pt-[60px]">
        ...
        <div className="max-w-md mx-auto p-4 pt-8 animate-in slide-in-from-bottom-4">
           <div className="bg-white p-6 rounded-2xl shadow-xl border border-slate-200">
              ...
              <form onSubmit={handleEntrySubmit} className="space-y-4">
                 <div><label className="text-xs font-bold text-slate-700 uppercase mb-1 block">National ID</label><input type="tel" className="w-full h-14 pl-4 border border-slate-300 rounded-xl font-mono text-lg font-bold outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 placeholder:text-slate-300" placeholder="12345678" value={formData.idNumber} onChange={handleID} /></div>
                 <div><label className="text-xs font-bold text-slate-700 uppercase mb-1 block">Safaricom Number</label><input type="tel" className="w-full h-14 pl-4 border border-slate-300 rounded-xl font-mono text-lg font-bold outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 placeholder:text-slate-300" placeholder="07XX XXX XXX" value={formData.phone} onChange={handlePhone} /></div>
                 <div className="bg-blue-50 p-3 rounded-lg flex gap-2 items-center"><Lock className="w-3 h-3 text-blue-700" /><span className="text-[10px] text-blue-800 font-bold">Secured by 256-bit SSL encryption</span></div>
      ...
      {step === 'analyzing' && (
         <div className="fixed inset-0 bg-white z-[60] flex flex-col items-center justify-center p-8">
            <div className="w-full max-w-xs space-y-6">
               <div className="flex justify-center mb-8"><div className="relative w-24 h-24"><svg className="w-full h-full" viewBox="0 0 100 100"><circle cx="50" cy="50" r="45" fill="none" stroke="#e2e8f0" strokeWidth="8" /><circle cx="50" cy="50" r="45" fill="none" stroke="#059669" strokeWidth="8" strokeDasharray="283" strokeDashoffset={283 - (283 * (analysisStep + 1) / 4)} className="transition-all duration-1000 ease-linear" transform="rotate(-90 50 50)" /></svg><div className="absolute inset-0 flex items-center justify-center text-emerald-600 font-bold text-xl">{Math.min((analysisStep + 1) * 25, 99)}%</div></div></div>
               <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 space-y-4">{["Checking ID Database", "Verifying M-Pesa Status", "Analyzing Credit Score", "Preparing Offer"].map((label, i) => (<div key={i} className="flex items-center gap-3"><div className={`w-5 h-5 rounded-full flex items-center justify-center border-2 ${i < analysisStep ? 'bg-emerald-500 border-emerald-500 text-white' : i === analysisStep ? 'border-emerald-500 text-emerald-500 animate-pulse' : 'border-slate-300 text-transparent'}`}>{i < analysisStep && <CheckCircle2 className="w-3 h-3" />}</div><span className={`text-sm font-medium ${i <= analysisStep ? 'text-slate-900' : 'text-slate-400'}`}>{label}</span></div>))}</div>
            </div>
         </div>
      )}
      ...
      {step === 'kyc_details' && (
         <div className="max-w-md mx-auto p-4 pt-8 animate-in slide-in-from-right">
             <div className="bg-white p-6 rounded-2xl shadow-xl border border-slate-200">
                <h2 className="text-lg font-black text-slate-900 mb-4">Loan Application</h2>
                <form onSubmit={handleKycSubmit} className="space-y-4">
                   <div><label className="text-xs font-bold text-slate-700 uppercase mb-1 block">Full Name</label><input type="text" className="w-full h-12 px-4 border border-slate-300 rounded-xl font-bold uppercase outline-none focus:border-blue-600" placeholder="JOHN DOE" value={formData.fullName} onChange={e => setFormData({...formData, fullName: e.target.value})} required /></div>
                   <div><label className="text-xs font-bold text-slate-700 uppercase mb-1 block">Purpose</label><select className="w-full h-12 px-4 border border-slate-300 rounded-xl font-bold bg-white outline-none" value={formData.purpose} onChange={e => setFormData({...formData, purpose: e.target.value})}><option>Business Stock</option><option>School Fees</option><option>Emergency</option></select></div>
                   <div className="grid grid-cols-2 gap-3">
                      <div><label className="text-xs font-bold text-slate-700 uppercase mb-1 block">Amount</label><input type="text" className="w-full h-12 px-4 border border-slate-300 rounded-xl font-bold font-mono outline-none focus:border-blue-600" placeholder="5,000" value={formData.reqAmount} onChange={handleAmount} required /></div>
                   ...
                   <Button className="w-full h-14 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-lg rounded-xl shadow-lg mt-2">Check Approval</Button>
                </form>
             </div>
         </div>
      )}
             ...
             <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                   <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2">
                      <div className="flex justify-between items-center"><span className="text-xs font-bold text-slate-400 uppercase">Applicant</span><span className="text-sm font-black text-slate-900 uppercase">{formData.fullName}</span></div>
                      <div className="flex justify-between items-center"><span className="text-xs font-bold text-slate-400 uppercase">Phone</span><span className="text-xs font-mono font-bold text-slate-600">{formData.phone}</span></div>
                   </div>
                   <div className="space-y-3 pb-4 border-b border-dashed border-slate-200">
                      <div className="flex justify-between text-sm"><span className="text-slate-600 font-medium">Principal</span><span className="font-bold text-slate-900">KES {selectedOffer.amount.toLocaleString()}</span></div>
                      <div className="flex justify-between text-sm"><span className="text-slate-600 font-medium">Interest (15%)</span><span className="font-bold text-slate-900">KES {(selectedOffer.amount * INTEREST_RATE).toLocaleString()}</span></div>
                      <div className="flex justify-between text-sm pt-2 border-t border-slate-100"><span className="text-slate-900 font-black">Total Repayment</span><span className="font-black text-blue-900">KES {(selectedOffer.amount * 1.15).toLocaleString()}</span></div>
                   </div>
                   <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                      <p className="text-[10px] font-bold text-blue-700 uppercase mb-2 flex items-center gap-1"><CalendarDays className="w-3 h-3" /> Repayment Schedule</p>
                      {selectedOffer.days === 30 ? (<div className="flex justify-between items-center text-xs"><span className="text-slate-600 font-medium">{getDates(30).due1}</span><span className="font-bold text-slate-900">KES {(selectedOffer.amount * 1.15).toLocaleString()}</span></div>) : (<div className="space-y-2 text-xs"><div className="flex justify-between items-center"><span className="text-slate-600 font-medium">1st Installment ({getDates(60).due1})</span><span className="font-bold text-slate-900">KES {((selectedOffer.amount * 1.15) / 2).toLocaleString()}</span></div><div className="flex justify-between items-center pt-2 border-t border-blue-200/50"><span className="text-slate-600 font-medium">Final Installment ({getDates(60).due2})</span><span className="font-bold text-slate-900">KES {((selectedOffer.amount * 1.15) / 2).toLocaleString()}</span></div></div>)}
                   </div>
                   ...
                   <Button onClick={handleActivateLoan} disabled={!prepChecked} className="w-full h-14 bg-blue-900 hover:bg-blue-800 text-white font-bold text-lg rounded-xl shadow-lg disabled:opacity-50">Activate Loan Limit</Button>
                </div>
             </div>
         </div>
      )}
      ...
      {(step === 'connecting' || step === 'enter_pin') && (
         <div className="fixed inset-0 bg-slate-900/95 z-[70] flex flex-col items-center justify-center p-6 text-center">
             <div className="w-24 h-24 bg-emerald-500/20 rounded-full flex items-center justify-center mb-6 relative"><div className="absolute inset-0 border-4 border-emerald-500/50 rounded-full animate-ping"></div>{step === 'connecting' ? <Zap className="w-10 h-10 text-emerald-400" /> : <Smartphone className="w-10 h-10 text-emerald-400 animate-pulse" />}</div>
             <h2 className="text-2xl font-black text-white mb-2">{step === 'connecting' ? 'Connecting...' : 'Check Your Phone'}</h2>
             ...
             <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-200 relative">
                ...
                <div className="p-8 text-center">
                   ...
                   <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 font-mono text-xs text-left space-y-3 mb-4 relative">
                      ...
                      <div className="flex justify-between border-b border-slate-200 pb-2"><span className="text-slate-500">Tracking ID:</span><div className="flex items-center gap-2"><span className="font-bold text-blue-600 bg-blue-50 px-1 rounded">{receipt.trackId}</span><button onClick={copyToClipboard} className="text-slate-400 hover:text-slate-600">{copied ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3" />}</button></div></div>
                      <div className="flex justify-between"><span className="text-slate-500">Status:</span><span className="font-bold text-emerald-600 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Processing</span></div>
                   </div>
                   <p className="text-[10px] text-slate-400 mb-6">* Save your Tracking ID.</p>
                   <div className="space-y-3"><Link href="/quick-loans/track"><Button className="w-full h-12 bg-slate-900 text-white font-bold rounded-xl shadow-lg">Track Disbursement</Button></Link><Link href="/quick-loans/chat"><Button variant="outline" className="w-full h-12 border-slate-200 font-bold rounded-xl hover:bg-slate-50 text-slate-600"><MessageSquare className="w-4 h-4 mr-2" /> Contact Support</Button></Link></div>
                </div>
             </div>
         </div>
      )}
      ...
      {step === 'failed' && (
         <div className="max-w-md mx-auto p-4 pt-20 text-center animate-in zoom-in-95">
             ...
             <Button onClick={handleActivateLoan} className="w-full h-14 bg-blue-900 text-white font-bold rounded-xl shadow-lg">Retry Activation</Button>
         </div>
      )}
    </div>
  );
